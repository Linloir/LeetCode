...
//Solution2 : Uses stack to reverse
...
int** levelOrderBottom(struct TreeNode* root, int* returnSize, int** returnColumnSizes){
    ...
    *returnColumnSizes = (int*)malloc(sizeof(int) * (*returnSize));
    int** resStack = (int**)malloc(sizeof(int*) * 1000);
    int* colStack = (int*)malloc(sizeof(int) * 1000);
    int stackSize = 0;
    if(!root){
        return NULL;
    }
    ...
    while(QueueSize(queue) > 0){
        ...
        res = (int**)realloc(res, sizeof(int*) * (stackSize + 1));
        *returnColumnSizes = (int*)realloc(*returnColumnSizes, sizeof(int) * (stackSize + 1));
        resStack[stackSize] = level;
        colStack[stackSize] = size;
        stackSize++;
    }
    for(int i = 0; i < stackSize; i++){
        res[i] = resStack[stackSize - i - 1];
        (*returnColumnSizes)[i] = colStack[stackSize - i - 1];
    }
